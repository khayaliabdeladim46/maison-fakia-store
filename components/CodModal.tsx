...
import { X, CheckCircle2, Truck, Phone, User, MapPin } from 'lucide-react';
...
export default function CodModal({ product, isOpen, onClose, lang = 'fr' }: CodModalProps) {
    ...
    // Safe String Extractor (كايضمن ما يطيحش الكود حتى إيلا جا Object)
    const currentLang = (lang === 'ar' ? 'ar' : 'fr') as 'fr' | 'ar';
    const nameStr = typeof product.name === 'string'
        ? product.name
        : (product.name?.[currentLang] || product.name?.fr || product.name?.ar || '');
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fadeIn">
            <div className="bg-white rounded-2xl max-w-md w-full overflow-hidden shadow-2xl border border-slate-100 relative">
                ...
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 z-10 w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 flex items-center justify-center transition cursor-pointer"
                    aria-label="Fermer"
                >
                    <X size={18} />
                </button>
                ...
                {isSubmitted ? (
                    <div className="p-8 text-center space-y-4">
                        ...
                        <p className="text-xs text-slate-600 leading-relaxed font-medium">
                            {lang === 'ar'
                                ? 'شكراً لك! سيتصل بك فريقنا قريباً لتأكيد عنوان التسليم.'
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#D97706] bg-[#D97706]/10 px-2.5 py-1 rounded-full">
                {lang === 'ar' ? 'الدفع عند الاستلام' : 'Paiement à la Livraison (COD)'}
              </span>
                            <h3 className="text-lg font-extrabold text-[#1E3A2B] pt-1">
                                {lang === 'ar' ? 'أدخل معلوماتك للطلب' : 'Finalisez Votre Commande'}
                            </h3>
                        </div>

                        {/* Product Summary */}
                        <div className="bg-[#FDFBF7] p-3.5 rounded-xl border border-slate-100 flex items-center gap-3">
                            <div className="w-14 h-14 relative shrink-0 bg-white rounded-lg p-1 border border-slate-100 flex items-center justify-center">
                                <Image src={product.image} alt={nameStr} width={50} height={50} className="object-contain max-h-12" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <h4 className="text-xs font-bold text-[#1E3A2B] truncate">{nameStr}</h4>
                                <p className="text-[10px] text-slate-500 font-medium truncate">{product.weight}</p>
                            </div>
